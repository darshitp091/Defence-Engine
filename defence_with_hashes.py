"""
Defence Engine - With Real-time Hash Display
"""
import sys
import os
import time
...
class DefenceEngineWithHashes:
    ...
    def start(self):
        """Start Defence Engine with hash display"""
        print("Defence Engine v1.0 - Hash Display Mode")
        ...
        print("License validated. Starting protection with hash display...")
        
        # Initialize components with hash display enabled
        try:
            self.quantum_engine = QuantumHashEngine(display_hashes=True)
            ...
            print("Real-time quantum hashes will be displayed below:")
            print("=" * 80)
    ...
    def _protection_loop(self):
        """Main protection loop with status updates"""
        try:
            while self.is_running:
                ...
                if self.quantum_engine:
                    ...
                    print(f"\n{'='*80}")
                    print(f"STATUS: {total_hashes:,} hashes generated | {threat_count} active threats")
                    print(f"{'='*80}")
...
def main():
    engine = DefenceEngineWithHashes()
    engine.start()
